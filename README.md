...
| `LINE_GROUP_ID` | 群組 ID（保留供未來使用；目前群組推播已停用） | 群組內指令 |
| `GROUP_NOTIFY_ENABLED` | ⚠️ **已停用**：所有系統推播一律只給主管私訊 | — |
...
> **國定假日**（依 Google Sheet「國定假日」分頁）不需傳工作日誌，僅需發布回報；主管可自行維護該分頁（YYYY/MM/DD｜名稱），10 分鐘內自動生效。週產能檢查會自動折算：本週落在國定假日的工作日會從標準中扣除。
