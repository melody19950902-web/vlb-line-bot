...
#### 分頁 6：`請假記錄`
小編傳「今日病假／明日特休…」時自動寫入；22:30 彙整與週六查核讀取當日請假名單。
在 A1 填入以下標題：
```
請假日期  提交時間  姓名  LINE_User_ID  類型  時數
```
...
在 A1 填入以下標題：
```
日期  時間  姓名  LINE_User_ID  原始文字
```

#### 分頁 8：`剪輯進度`
小編傳「剪輯進度｜影片標題｜狀態」時自動寫入。
在 A1 填入以下標題：
```
日期  時間  姓名  LINE_User_ID  影片標題  狀態
```
...
> 分頁名稱必須完全一致。分頁 6~10 若沒建立，系統不會當掉，但對應功能會靜默失效（請假／發布／剪輯進度記不進去、月底統計無法防重複）。
...
**發布回報（每天發布完成後傳，週一～週六都要）：**
```
已發布｜IG｜帳號名稱
```
每日 22:30 彙整會標記當日未回報發布的人；週六 22:30 專門查核發布。
...
*(3) 寫在工作日誌裡：*
- 時間記錄放一行 `09:00-11:00 補休`
- 或在時間記錄／備註內文寫 `補休 2 小時`、`補休兩小時`、`補休半天`

系統會自動扣除該時段工時、按比例查核工時與影片數，並自動補一筆請假記錄。
...
**也聽得懂自然說法**（例：`我明天要特休`、`8/5事假`、`下週一補休半天`、`後天休假`、`8月5日病假`），但**建議用標準格式最穩**。錯字或閒聊時 bot 會請你補正確資訊，不會靜默忽略；純聊天不會被誤判為請假。
...
| `本週請假` | 本週誰請假一覽 |
| `本月請假` | 本月每人請假統計 |
| `本月剪輯統計` | 本月每人剪輯支數合計（獎金核對用） |
| `上月剪輯統計` | 上月每人剪輯支數合計（獎金核對用） |
| `@小芯 狀況` | 查詢特定小編本週記錄 |
| `說明` | 顯示所有指令與格式 |
